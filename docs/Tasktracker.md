...
## Задача: 📊 GOOGLE SHEETS ИНТЕГРАЦИЯ: Отображение заказов в реальном времени
- **Статус**: 📋 Планируется
- **Дата**: 2025-01-28  
- **Приоритет**: 🏆 ВЫСОКИЙ
- **Описание**: Реализовать интеграцию с Google Sheets API для автоматического добавления всех заказов в Google таблицу в реальном времени. Новые заказы должны появляться наверху таблицы, а старые опускаться вниз.

### 🎯 Бизнес-цели
- **Прозрачность**: Централизованный просмотр всех заказов в удобном формате
- **Аналитика**: Возможность анализа данных в Google Sheets с помощью формул и графиков
- **Интеграция**: Совместимость с внешними системами через Google Sheets API
- **Мониторинг**: Отслеживание заказов и платежей в реальном времени

### 🏗️ Техническая архитектура
- **Event-driven подход**: Использование существующей системы событий (NewOrderEvent, PaymentStatusChangedEvent)
- **Асинхронная обработка**: Отправка данных в Google Sheets без блокировки основных операций
- **Retry механизм**: Повторные попытки при сбоях API Google Sheets
- **Конфигурируемость**: Ссылка на таблицу и учетные данные через Docker Compose

### 📋 Структура данных в таблице
**Колонки таблицы (A-P)**:
1. **A**: ID заказа
2. **B**: Дата и время создания  
3. **C**: Имя клиента
4. **D**: Телефон клиента
5. **E**: Email клиента (если есть)
6. **F**: Состав заказа (товары с количеством)
7. **G**: Адрес доставки
8. **H**: Тип доставки (Самовывоз/Курьер)
9. **I**: Стоимость товаров (₽)
10. **J**: Стоимость доставки (₽)
11. **K**: Общая сумма (₽)
12. **L**: Способ оплаты (Наличными/СБП/Карта)
13. **M**: Статус платежа (Не оплачен/Оплачен/Отменен)
14. **N**: Статус заказа (Создан/Подтвержден/Готовится/Доставляется/Выполнен)
15. **O**: Комментарий к заказу
16. **P**: Ссылка на платеж ЮКасса (если есть)
...
public class GoogleSheetsService {
    ...
    // Метод добавления строки в начало таблицы  
    // Метод обновления статуса заказа
    // Retry логика при ошибках API
}
```

#### 2. **GoogleSheetsEventListener**
```java
@EventListener
public class GoogleSheetsEventListener {
    // Обработка NewOrderEvent -> добавление нового заказа
    // Обработка OrderStatusChangedEvent -> обновление статуса
    // Обработка PaymentStatusChangedEvent -> обновление платежа
}
```

#### 3. **GoogleSheetsConfiguration**
```java
@ConfigurationProperties("google.sheets")
public class GoogleSheetsConfiguration {
    // Ссылка на таблицу
    // Путь к credentials файлу
    // Настройки API (timeouts, retries)
}
```
...
**Шаги выполнения**:
  - [ ] 🔧 Настройка Google Sheets API и создание Service Account
  - [ ] 📦 Добавление зависимостей Google Sheets API в build.gradle
  - [ ] ⚙️ Создание GoogleSheetsService с базовым функционалом
  - [ ] 🎯 Реализация метода добавления заказа в начало таблицы
  - [ ] 📊 Форматирование данных заказа для Google Sheets
  - [ ] 🔄 Создание GoogleSheetsEventListener для обработки событий
  - [ ] 🔗 Интеграция с существующими событиями заказов и платежей
  - [ ] 🐳 Настройка переменных окружения в Docker Compose
  - [ ] 🧪 Создание тестов интеграции с Google Sheets
  - [ ] 📖 Обновление документации проекта
  - [ ] 🎨 Создание заголовков и форматирования таблицы
  - [ ] ⚡ Оптимизация производительности (batch операции)

### 🔒 Безопасность
- **Service Account**: Использование Google Service Account для авторизации
- **Credentials**: Безопасное хранение credentials.json в Docker Compose
- **Права доступа**: Минимальные права только на конкретную таблицу
- **Валидация**: Проверка корректности данных перед отправкой

### 📊 Особенности реализации
- **Порядок записей**: Новые заказы добавляются в строку 2 (после заголовков)
- **Обновление статусов**: Поиск и обновление существующих записей по ID заказа
- **Асинхронность**: Операции с Google Sheets не блокируют основной процесс
- **Обработка ошибок**: Graceful fallback при недоступности Google Sheets API

### 🧪 Тестирование
- **Создание тестовой таблицы** для разработки
- **Unit тесты** для GoogleSheetsService
- **Integration тесты** с реальным Google Sheets API  
- **E2E тесты** полного цикла: заказ → таблица → обновление статуса
...
### 🎁 Результат
- ✅ **Реальное время**: Все заказы мгновенно попадают в Google таблицу
- ✅ **Полная информация**: Все данные заказа, доставки и платежей в одной строке
- ✅ **Удобство**: Новые заказы всегда наверху для быстрого просмотра
- ✅ **Интеграция**: Бесшовная работа с существующей системой событий
- ✅ **Надежность**: Retry механизм и graceful fallback при сбоях
- ✅ **Аналитика**: Возможность создания отчетов и графиков в Google Sheets

**Связанные компоненты**: OrderService, YooKassaPaymentService, AdminBotService, Spring Events, Docker Compose
