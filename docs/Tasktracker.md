...
- **Шаги выполнения**:
  - [x] Добавить переменные окружения для управления ботами в docker-compose.yml
  - [x] Добавить переменные окружения для управления ботами в docker-compose.dev.yml
  - [x] Обновить TelegramBotConfig.java для поддержки условного включения ботов
  - [x] Обновить PizzaNatTelegramBot.java с аннотацией @ConditionalOnProperty
  - [x] Обновить PizzaNatAdminBot.java с аннотацией @ConditionalOnProperty
  - [x] Обновить TelegramBotService.java для проверки включенности ботов
  - [x] Обновить AdminBotService.java для проверки включенности админского бота
  - [x] Создать документацию по управлению ботами
  - [x] Протестировать отключение/включение ботов в dev и prod режимах
...
- **Шаги выполнения**:
  - [x] Удалить зависимость springdoc-openapi-starter-webmvc-ui из build.gradle
  - [x] Удалить OpenApiConfig.java
  - [x] Удалить все аннотации @Operation, @Tag, @Schema из контроллеров
  - [x] Удалить все аннотации @ApiResponse, @Parameter из контроллеров
  - [x] Удалить все импорты io.swagger.v3.oas.annotations.*
  - [x] Обновить HomeController.java - убрать перенаправление на swagger
  - [x] Удалить swagger-related настройки из application.properties
  - [x] Удалить swagger-related настройки из application.yml
  - [x] Обновить SecurityConfig.java - убрать swagger пути из whitelist
  - [x] Обновить документацию проекта
  - [x] Протестировать запуск приложения без Swagger
- **Зависимости**: Spring Boot, Security конфигурация
- **Детальный план**:
...
- **Шаги выполнения**:
  - [x] Удалить зависимость spring-boot-starter-data-redis из build.gradle
  - [x] Удалить RedisConfig.java
  - [x] Удалить TestRedisConfig.java
  - [x] Обновить CacheConfig.java - оставить только ConcurrentMapCacheManager
  - [x] Удалить Redis настройки из application.properties
  - [x] Удалить Redis настройки из application.yml
  - [x] Удалить Redis настройки из application-*.properties файлов
  - [x] Удалить Redis сервис из docker-compose.dev.yml
  - [x] Удалить Redis сервис из docker-compose.base.yml
  - [x] Обновить зависимости в docker-compose файлах
  - [x] Удалить Redis переменные окружения из всех compose файлов
  - [x] Обновить кэширование сервисов на использование простого кэша
  - [x] Протестировать работу кэширования без Redis
...
- **Шаги выполнения**:
  - [x] Диагностировать проблему с токенами в TelegramAuthService
  - [x] Исправить логику создания и поиска токенов в БД
  - [x] Улучшить обработку ошибок в TelegramAuthController
  - [x] Исправить логику подтверждения номера телефона в PizzaNatTelegramBot
  - [x] Обновить TelegramWebhookService для корректной обработки контактов
  - [x] Добавить детальное логирование процесса авторизации
  - [x] Исправить race conditions в процессе авторизации
  - [x] Улучшить валидацию токенов и пользовательских данных
  - [x] Добавить автоматическую очистку истекших токенов
  - [x] Создать механизм повторной авторизации при ошибках
  - [x] Обновить мобильное приложение для корректного отображения пользователя
  - [x] Протестировать полный цикл авторизации через Telegram
...
- **Шаги выполнения**:
  ...
  - [x] Обновить технологический стек в документации
  - [x] Пересмотреть и оптимизировать конфигурации Spring Boot
  - [x] Обновить Dockerfile для уменьшения размера образа
  - [x] Оптимизировать зависимости в build.gradle
  - [x] Обновить тесты после удаления компонентов
  - [x] Создать новые health checks без Swagger
  - [x] Обновить мониторинг и логирование
  - [x] Пересмотреть security конфигурацию
  - [x] Обновить CI/CD процессы
...
- **Результат**: Все 4 критических проблемы исправлены и протестированы. Пользователи могут успешно авторизоваться через Telegram и использовать мобильное приложение.
...
- [x] ✅ Включить Telegram Auth в production (`TELEGRAM_AUTH_ENABLED: true`)
- [x] ✅ Проверить и настроить SMS авторизацию (Exolve API)
- [x] ✅ Решить конфликт Long Polling vs Webhook
- [x] ✅ Протестировать авторизацию с мобильного приложения
- [x] ✅ Обновить документацию по мобильной интеграции
