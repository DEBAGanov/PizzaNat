...
## [2025-06-13] - Исправление проблемы с пользователями Telegram
...
- **Проблема "Пользователь не авторизован"** при создании заказов через Telegram аутентификацию
...
- **Улучшенная обработка данных** в `TelegramUserDataExtractor`
- **Тестовый скрипт** `test_telegram_user_fix.sh` для проверки исправления

### Изменено
- **TelegramUserDataExtractor.createUserFromTelegramData()** - добавлена генерация email и активация пользователя
- **TelegramUserDataExtractor.updateUserWithTelegramData()** - добавлена проверка и генерация email для существующих пользователей
- **Логика создания пользователей** - обеспечена полная совместимость с мобильным приложением

### Технические детали
- **Генерация email**: `tg_{telegram_id}@telegram.pizzanat.local` для уникальности
- **Автоматическая активация**: `isActive(true)` при создании пользователя
- **Обратная совместимость**: существующие пользователи обновляются через миграцию
- **Безопасность**: email генерируется только для Telegram пользователей без существующего email
