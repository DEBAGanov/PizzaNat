...
## [2025-01-26] - 🔧 ИСПРАВЛЕНИЕ: Поле payment_status в Order entity

### Исправлено
- **Критическая проблема**: Добавлено отсутствующее поле `payment_status` в `Order` entity
- **Синхронизация БД**: Поле `payment_status` теперь корректно обновляется при успешной оплате
- **YooKassaPaymentService**: Обновляет `payment_status = PAID` при webhook событиях
- **PaymentPollingService**: Обновляет `payment_status = PAID` при опросе статуса платежей

### Добавлено
- **OrderPaymentStatus enum**: Статусы оплаты заказов (UNPAID, PAID, PENDING, CANCELLED, FAILED)
- **Логирование**: Детальные логи обновления статуса оплаты

### Техническая информация
**Проблема**: В БД поле `payment_status` создавалось миграцией V18, но отсутствовало в Java entity, поэтому всегда оставалось `UNPAID`. JPA не может обновлять поля, которых нет в entity.

**Решение**: 
- Добавлено поле `paymentStatus` в `Order.java`
- Создан enum `OrderPaymentStatus` 
- Обновлена логика в сервисах для установки статуса `PAID` при успешной оплате

**Затронутые файлы**:
- `src/main/java/com/baganov/pizzanat/entity/Order.java`
- `src/main/java/com/baganov/pizzanat/entity/OrderPaymentStatus.java` (новый)
- `src/main/java/com/baganov/pizzanat/service/YooKassaPaymentService.java`
- `src/main/java/com/baganov/pizzanat/service/PaymentPollingService.java`
...
- `sendPaymentCancelAlert()` - алерт об отменённом платеже  
